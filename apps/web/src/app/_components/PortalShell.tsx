...
export default function PortalShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="h-screen flex overflow-hidden flex-col">
      {/* Global active jobs banner */}
      {activeJobs > 0 && (
        <div className="shrink-0 bg-blue-600 text-white text-xs py-2 px-4 flex items-center justify-between z-50">
          <div className="flex items-center gap-2">
            <span className="inline-block w-2 h-2 rounded-full bg-white animate-pulse"></span>
            <span>
              มีงานกำลังดำเนินการอยู่{' '}
              <strong>{activeJobs} งาน</strong>{' '}
              — ระบบกำลังประมวลผล
            </span>
          </div>
          <Link href="/jobs" className="underline hover:no-underline font-medium">ดูรายละเอียด →</Link>
        </div>
      )}
